...
impl SchedulerService {
    ...
    async fn perception_scan(&self) -> Result<usize, String> {
        ...
        // Also flag if there are many unreviewed due notes with no consolidation task pending.
        ...
        if due_count >= 10 {
            let consolidate_check = neo4rs::query(
                "MATCH (t:Task) \
                 WHERE t.goal CONTAINS 'consolidate' \
                   AND t.status IN ['created', 'in_progress'] \
                 RETURN count(t) AS cnt",
            );
            let existing: i64 = self.neo4j
                .execute(consolidate_check)
                .await
                .ok()
                .and_then(|rows| rows.first().and_then(|r| r.get::<i64>("cnt").ok()))
                .unwrap_or(0);
            ...
            if existing == 0 {
                ...
                if let Ok(_) = self.neo4j.create_task(
                    &goal,
                    Some("Auto-generated by proactive perception scan (spaced-repetition backlog)"),
                ).await {
                    created += 1;
                    info!(due_count = due_count, "Perception scan created consolidation task");
                }
            }
        }
