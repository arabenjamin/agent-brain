...
mod tests {
    ...
    fn test_aws_config_builder() {
        let config = AwsSecretConfig::new()
            ...
            .with_prefix("/agent-api");

        assert_eq!(config.region, Some("us-west-2".to_string()));
        assert_eq!(config.prefix, "/agent-api");
    }

    #[test]
    fn test_build_secret_name() {
        let config = AwsSecretConfig::new().with_prefix("/agent-api");
        let provider = AwsSecretProvider::new(config);

        assert_eq!(
            provider.build_secret_name("openweathermap/key"),
            "/agent-api/openweathermap/key"
        );
    }
    ...
    fn test_strip_prefix() {
        ...
        assert_eq!(
            provider.strip_prefix("/agent-api/openweathermap/key"),
            "openweathermap/key"
        );
    }
