...
- **Phase status:** Phases 1–4 implemented (captions, on-demand, autonomous YouTube watch, learning loop, and self-hosted Whisper fallback via `services/transcribe.rs` + the `whisper` sidecar). Phase 5 (podcast RSS + local files) is still a stubbed seam — `feed_url` returns `None` for `podcast_rss` and `validate_url` blocks `file://`; both would reuse the Phase-4 transcriber once wired.
