...
**Routing:** `models.yaml` defines a `computation` capability held by **`qwen2.5-coder:7b` alone**. `select_models` orders cost ASC then context-window DESC and every local/ollama-cloud model is $0 — so the moment a second $0 model claims `computation`, the largest-context cloud model silently takes every computation step. Treat the capability as a routing selector, not a description of aptitude. VRAM: 4.68 GB on an 8 GB card already holding gemma4 + Whisper, so it swaps rather than co-resides; keep computation steps rare and batched.
