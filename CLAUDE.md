...
| `SERPAPI_KEY` | - | SerpApi key for `search_web` tool. Free tier is **250 searches/month** (re-verified 2026-09-04; the earlier "100" was wrong). Backs both the `serpapi` (Google organic) and `google_news` engines, which **share one account quota**. The daily news brief now requests `engine: google_news` on its 7 news searches (~210/month), so this is a primary for news; general/background mining stays on unmetered SearXNG |
...
**The news sections were fixed on 2026-09-04 by routing the brief through SerpApi's Google News engine.** The chain of prior notes above kept concluding that "the working [SearXNG] engines cannot serve fresh news" and deferring to a Google-CSE key that was never configured. The actual fix was to stop asking a general-purpose metasearch for news and use a news-specific engine: `search_web` now exposes `engine: "google_news"` (`search_serpapi_news` in `skills/search.rs`), which hits SerpApi's `engine=google_news` and returns curated, dated stories with a named source. The daily brief requests it on all 7 news steps; it is promoted to the head of the ladder and still falls back through SearXNG when the quota is spent, so no step can hard-fail on it.

The quota math is now survivable because two earlier premises were wrong. **SerpApi's free tier is 250/month, not 100** (re-verified 2026-09-04) — the "100" in every note above is stale. At 7 news searches/day the brief is ~210/month, under 250 with headroom for the graceful fallback on a heavy day. Two engine names (`serpapi`, `google_news`) draw on **one** account, so `engine_ladder` demotes both when either records `quota_exhausted` — otherwise the ladder would keep hammering a spent account through the other name.

`source_list` is deliberately **not** set on the Google News steps: appending `(site:… OR …)` to a query degrades Google News badly, and Google News already curates its sources — which also cured the metro-Detroit/Michigan junk-results problem the site-restricted SearXNG queries had. `google_news` is **not** in the default `SEARCH_ENGINE_ORDER` ladder; only a caller that sets `engine: google_news` reaches it, so general/background data mining is untouched and keeps leading with unmetered SearXNG. Configuring `GOOGLE_API_KEY` + `GOOGLE_CX` (Custom Search, 100 free queries/day) would still add a redundant keyed rung for general search, but is no longer the blocker for the news brief.
